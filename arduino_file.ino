...
void setup() {
  ...
  Serial.begin(115200);                                //Start the serial connetion @ 9600bps
...
void get_inst(){
  read_from_serial();//to fix later; affets serial values to code_ins and parameter
  if(code_ins!=0)
  {
    if(code_ins==1){
      gyro_type=parameter;
    ...
    if(code_ins==7){
      ...
      else if (parameter>=0 && parameter<= 100) {kalman_filter_activated=true ;kalman_filter_coeff=parameter/100;}
    };
    if(code_ins==15){
